...
<html >
  <!-- ⟨0⟩ Actually load Vue.js -->
  <script src="https://unpkg.com/vue@3"></script>
  <body>
      <div id="hello">  <!-- ⟨1⟩ Vue.js is active for this div; see ⟨4⟩ -->
      ...
      <!-- ⟨4⟩ Setup the associated model for “#hello” -->
      <script>
       ...
       // Here, we can perform complex logic (JS code) on “myApp.yourName”.
...
<html>
  ...
  <body>
    <div id="app">
      ...
      <!-- VARIABLES [Not reccommended, better use “data” below.] ---------------->
      <!-- Let's declare a var, but hide the resulting value from view -->
      <p hidden>  {{  love = 'JavaScript'  }}  </p>
      <!-- Use the variable -->
      {{ love + " is cool" }} <br> <br>

      <!-- APPLICATION: Keeping count. Definitions are in “data” and “methods” below. -->
      <button v-on:click="increment">        Increment      </button>
      Count: {{count}}

      <!-- Inline multiple actions; only works with proper v-model variables; i.e. not “love” above. -->
      <!-- “age” is declared below in “data”. -->
      ...
      <!-- “Hello” is shown whenever the above ‘age’ input is truthy/non-zero. -->
    ...
    <!-- PUTTING IT ALL TOGETHER: A TODO APP IN ≈30 LINES --------------------------------->
    <hr>
    <div id="todo-app">
    ...
    <!-- Always refers to whatever is CURRENTLY the first item in “todos”. -->
...
src_html[:exports code]{<!-- TODO APP. v-bind:𝒳 ≈ :𝒳 -->}
Vue.js provides HTML directives that “already exist” but /know how to evaluate
...
:END:
** COMMENT Directives & Components ---Essentially “HTML Functions”
:PROPERTIES:
...
+ src_emacs-lisp[:exports code]{controller}: Furnish the directive with bindings
  ...
  - We /attached the src_emacs-lisp[:exports code]{letters} binding to the scope/
    using Vue.js's “dependency injection”: We get the scope of the current tag
    via the special ‘service’ src_emacs-lisp[:exports code]{$scope}.
...
[[https://docs.angularjs.org/tutorial/step_03][Components]] are a [[https://docs.angularjs.org/guide/component][‘saner’]] form of directives that aim to narrow scope.
...
The special src_emacs-lisp[:exports code]{$scope} ‘service’ is used to expose
...
#+latex: \vspace{0.4em}
 ...
 code]{ng-click}, controllers, and other directives, build a “number guessing game”; the user
...
# The “.controller”'s cannot be chained like the “.when”!
...
(with-temp-buffer
    ...
    ,*The listing sheet, as PDF, can be found
     ...
     ---there is also a
     badge:beautiful|HTML_webpage|success|https://alhassy.github.io/Vue.jsCheatSheet|javascript (•̀ᴗ•́)و
